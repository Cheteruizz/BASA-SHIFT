...
export default function AssistantPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "bot",
      text: "Hola. Soy BASA Shift.\n\nCuéntame cómo funciona tu bar y tu plantilla como si me lo dijeras por WhatsApp. Yo te iré preguntando solo lo que falte."
    }
  ]);
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-5xl flex-col">
      ...
      <main className="flex flex-1 flex-col overflow-hidden rounded-lg border border-cyanx/15 bg-[#061426]/95 brand-glow">
        <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.role === "owner" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[88%] whitespace-pre-wrap rounded-lg px-4 py-3 text-sm leading-6 shadow-sm ${
                message.kind === "error"
                  ? "border border-red-300 bg-red-50 text-red-800"
                  : message.role === "owner"
                    ? "bg-electric text-white shadow-electric/20"
          ...
          {phase === "approved" && (
            <div className="rounded-lg border border-cyanx/30 bg-ink/70 p-4">
              <h2 className="brand-title font-black text-snow">WhatsApp preparados</h2>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {whatsappLinks.map((item) => (
                  <a key={item.name} href={item.href} target="_blank" rel="noreferrer" className="rounded-lg bg-electric px-4 py-3 text-sm font-bold text-white">
                    Enviar a {item.name}
                  </a>
                ))}
              </div>
        ...
        <form className="flex gap-2 border-t border-cyanx/15 bg-ink/90 p-3 sm:p-4" onSubmit={(event) => { event.preventDefault(); submit(); }}>
          <input
            value={input}
            disabled={loading}
            onChange={(event) => setInput(event.target.value)}
            placeholder="Escribe como hablarías con una persona..."
            className="min-w-0 flex-1 rounded-lg border border-cyanx/20 bg-[#071B2F] px-4 py-3 text-snow outline-none placeholder:text-snow/45 focus:border-cyanx"
          />
          <button type="submit" disabled={loading} className="rounded-lg bg-electric px-5 py-3 text-sm font-black text-white shadow-lg shadow-electric/25">
            Enviar
          </button>
        </form>
      </main>
...
function scheduleSummary(schedule: ReturnType<typeof generateWeeklySchedule>) {
  ...
  return `${hours}\n\n${schedule.conflicts.length ? `Conflictos: ${schedule.conflicts.length}` : "Sin conflictos."}`;
}
